...
x.base_h <- filter(x.base,grepl('重症',discharge_diag))
...
x.base_m2 <- filter(x.base,grepl('出血|坏死|化脓',discharge_diag))
x.exe_m <- x.exe
p.base_m <- unique(c(x.base_m$patient_id,x.base_m2$patient_id))
p.exe_m <- unique(x.exe$patient_id)
p.mid <- table(c(p.base_m,p.exe_m))
