...
x.jg_se <- filter(x.jg2,patient_id.x!=patient_id.y) %>% as.data.frame()
...
system.time(
  jm <- lapply(p,function(i){
    ...
    i.rate <- i.rate %>% arrange(interval_days1,interval_days2) %>% mutate(
      rate = ifelse(item=='ICU',1,rate)
    )
    i.info$overall_rate <- max(c(i.rate$rate,i.info$com_or_not==1,i%in%x.med$patient_id))
    i <- rbind(i.base,i.idx,i.exe1,i.exe2) %>% arrange(date,status) %>% mutate(date=date/max(date))
